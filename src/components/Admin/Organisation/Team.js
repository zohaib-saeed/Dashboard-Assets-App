  ...
  return (
    <>
          ...
          <div className="w-full flex flex-col items-center justify-start">
            <div className="w-full grid grid-cols-2 sxl:grid-cols-3 gap-y-3 gap-2 md:gap-3 sxl:gap-4">
              {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((item, index) => (
                <div
                  key={index}
                  onClick={teamDetailViewHandler}
                  className="w-full h-auto cursor-pointer bg-mirage rounded-xl md:rounded-2xl sxl:rounded-3xl px-3 sm:px-5 py-3 sm:py-5 flex flex-col items-center justify-start gap-6 sm:gap-9 sxl:gap-7"
                >
      ...
      <TeamDetail
        ...
        setShowDetail={setShowTeamDetail}
      />
    </>
  );
};
